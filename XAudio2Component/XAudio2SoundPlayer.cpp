...
using namespace XAudio2Component;
...
bool XAudio2Component::XAudio2SoundPlayer::LoadWav(Platform::String^ appPackageRelativePath)
{
	try
	{
		SoundFileReader testSound(appPackageRelativePath);
		(void)this->AddSound(testSound.GetSoundFormat(), testSound.GetSoundData());
	}
	catch(...)
	{
		return false;
